...
# NOTE: Adapted for Ansible 2.6 compatibility from the upstream code.
# https://raw.githubusercontent.com/ansible/ansible/fbe9d7f32a99852cd952d7f4efb740069cbaf4bc/lib/ansible/modules/mount_facts.py
...
@functools.lru_cache(maxsize=None)
def list_uuids_linux() -> t.List[str]:
...
def gen_mounts_from_stdout(stdout: str) -> t.Iterable[MountInfo]:
    ...
    pattern = get_mount_pattern(stdout)
    if not (pattern):
        stdout = ""
...
def gen_fstab_entries(lines: t.List[str]) -> t.Iterable[MountInfo]:
...
def gen_vfstab_entries(lines: t.List[str]) -> t.Iterable[MountInfo]:
...
def gen_aix_filesystems_entries(lines: t.List[str]) -> t.Iterable[MountInfoOptions]:
...
def gen_mnttab_entries(lines: t.List[str]) -> t.Iterable[MountInfo]:
...
def gen_mounts_by_file(file: str):
...
def get_sources(module: AnsibleModule) -> t.List[str]:
...
def gen_mounts_by_source(module: AnsibleModule):
    ...
    for source in sources:
        ...
        if source in seen or (real_source) in seen:
            continue
