...
- `export`: str. weka export options, eg `"/scratch"` (default `weka_export`).
  Like in NFS, a single WEKA cluster can export multiple filesystems for clients to use.
...
it is the optimal settings for you.

**WARNING**: Ensuring WEKA and Slurm parameters match gets more complex because of hyperthreading:
...
- `slurmd` convert virtual CPU ID to physical one and expands to other threads on same core (on a 2 sockets, 16 core per socket system, virtual CPU 0,1 = CPU#0,CPU#32)

Example: on a 2 socket, 16 core per socket system, we reserve cores 22, 23 for WEKA, 21 for the system and slot 0.
