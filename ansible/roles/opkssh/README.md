...
4. By default, `sshd` and `ssh` are configured so that ssh connections from this
   host to itself use host-based authentication (instead of user keys or
   OIDC). This allows the OpenOndemand's web shell, which uses ssh, to work as
   normal.
...
2. Add login host(s) to the `opkssh` group, e.g.:
...
1. The ssh user `ssh user@...` should be their *Linux* user, as normal, not
   their OIDC identity.
