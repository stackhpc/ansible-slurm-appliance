...
data "openstack_images_image_v2" "login" {
  name = var.login_nodes["login-0"].image
}

data "openstack_images_image_v2" "compute" {
  name = var.compute_types["small"].image
}
...
resource "openstack_compute_instance_v2" "login" {
  ...
  block_device {
    ...
    # Create new volume to boot from    
    uuid = data.openstack_images_image_v2.login.id
    source_type  = "image"
    destination_type ="volume"
    volume_size = 50
    boot_index = 0
...
resource "openstack_compute_instance_v2" "compute" {
  ...
  block_device {
    ...
    # Create new volume to boot from
    uuid = data.openstack_images_image_v2.compute.id
    source_type  = "image"
    destination_type ="volume"
    volume_size = 50
    boot_index = 0
