...
resource "openstack_networking_port_v2" "compute" {
  ...
  name = "${var.cluster_name}-${each.key}"
  network_id = data.openstack_networking_network_v2.networks[each.value.network].id
  admin_state_up = "true"

  fixed_ip {
    subnet_id = data.openstack_networking_subnet_v2.subnets[each.value.subnet].id
  }
